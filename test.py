...
def start(host,port,headers):
    ...
    for __ in range(0,999,1):
        ...
        while is_start==0:
            time.sleep(2)
        for _ in range(0,9999999,1):
            try:
                #print('{0}{1}'.format(str(name),str(_)))
                passw=id_generator(3)
                #print(passw)
                body_bytes = 'uname={0}{1}&passw={2}&repassw={2}&captcha=1234'.format(str(name),str(_),str(passw)).encode('ascii')
